...
// These cases exercise the injector's own fallbacks (document title, host slug,
// appName, the og.grok.me placeholder). Without an explicit workspace they read
// THIS app's src/lib/og/site.json and public/og.jpg, whose real title and custom
// card replace the value each case is about. An empty workspace and an empty
// site keep them about the injector.
const EMPTY_WORKSPACE = mkdtempSync(join(tmpdir(), "grok-og-empty-"));
const isolated = (ctx = {}) => ({ cwd: EMPTY_WORKSPACE, site: {}, ...ctx });
...
test("emits og:image for a public host and prefers a custom card", () => {
  const placeholder = injectGrokPwaHead(
    "<html><head></head></html>",
    isolated({
      appName: "Wild Race",
      host: "wild-race.grok.me",
  ...
  const custom = injectGrokPwaHead(
    "<html><head></head></html>",
    isolated({
      appName: "Wild Race",
      host: "wild-race.grok.me",
...
test("placeholder og:image appends site.color when it is 6-digit hex", () => {
  const themed = injectGrokPwaHead(
    "<html><head></head></html>",
    isolated({
      host: "wild-race.grok.me",
  ...
  const invalid = injectGrokPwaHead(
    "<html><head></head></html>",
    isolated({
      host: "wild-race.grok.me",
