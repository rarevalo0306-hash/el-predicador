 ...
 * `ask_unavailable` (no key in the hosting), `ask_failed:<code>` (the
 * service; the code is short and safe to show, e.g. `deepseek_402`).
 */
export const askPreacher = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<AskReply> => {
    const { ASK_DAILY_LIMIT, answerQuestion, consumeAskQuota, failureCode, releaseAskQuota } =
      await import("./ai/ask.ts");
    ...
    try {
      ...
      console.error("[ask] answer failed:", code, error instanceof Error ? error.message : error);
      // The question did not get an answer, so it does not count.
      await releaseAskQuota(sql, context.userId).catch(() => undefined);
      throw new Error(`ask_failed:${code}`);
    }
  });
